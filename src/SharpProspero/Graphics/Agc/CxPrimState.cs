...
{
    /// <summary>Maximum number of primitive state context registers.</summary>
    public const int MaxRegisters = 2;
    ...
    public static CxPrimState Create(AgcShader vs, uint primitiveType = 4, AgcShader? gs = null, Action<string>? trace = null)
    {
        ...
        CxRegister* cxBuffer = stackalloc CxRegister[MaxRegisters];
        new Span<byte>(cxBuffer, sizeof(CxRegister) * MaxRegisters).Clear();
