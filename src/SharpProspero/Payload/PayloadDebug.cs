...
/// <c>ptrace</c>. The SDK <c>test_privileges</c> sample exercises both mechanisms to verify
/// that the payload has sufficient privileges to read and write another process's memory.
/// </summary>
/// <remarks>
...
/// credentials (authid + caps) to succeed, so the sample temporarily escalates credentials
/// before calling ptrace and restores them afterwards.</para>
/// <para>The wrapper exposes the raw libc <c>ptrace</c> entry point. Credential escalation
/// should be done through <see cref="PayloadKernel"/> before calling ptrace.</para>
/// </remarks>
public static unsafe partial class PayloadDebug
{
    ...
    /// (typically escalated via <see cref="PayloadKernel.SetUcredAuthId(int, ulong)"/> and
    /// <see cref="PayloadKernel.SetUcredCaps"/>).
    /// </summary>
    ...
    /// Corresponds to SDK <c>mdbg_copyout(pid, addr, buf, len)</c>.
    /// </summary>
    [SuppressGCTransition]
    ...
    /// Corresponds to SDK <c>mdbg_copyin(pid, buf, addr, len)</c>.
    /// </summary>
    [SuppressGCTransition]
    [LibraryImport(LibPosix)]
