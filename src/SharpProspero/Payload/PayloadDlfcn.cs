...
/// (<c>__dlopen</c>, <c>__dlsym</c>, <c>__dlclose</c>, <c>__dlerror</c>) which the SDK
/// <c>hello_dlfcn</c> and <c>hello_so</c> samples exercise. These functions are provided by
/// the CRT's <c>rtld_dlfcn</c> subsystem and resolve at link time through the CRT archive,
/// not through a DT_NEEDED SPRX.
/// </summary>
/// <remarks>
...
public static unsafe partial class PayloadDlfcn
{
    ...
    /// <summary>Search the default symbol scope (the payload itself and all loaded modules).</summary>
    public static readonly nint RtldDefault = (nint)0;

    /// <summary>
    /// Opens a shared object at runtime, returning a handle for symbol lookup.
    /// Corresponds to <c>dlopen("libSceRandom.sprx", RTLD_LAZY)</c> in the SDK
    /// <c>hello_dlfcn</c> sample.
    /// </summary>
    ...
    /// Corresponds to <c>dlsym(handle, "sceRandomGetRandomNumber")</c> in the SDK
    /// <c>hello_dlfcn</c> sample.
    /// </summary>
    /// <param name="handle">A handle from <see cref="Dlopen"/>, or <see cref="RtldDefault"/>
    /// to search the default scope.</param>
