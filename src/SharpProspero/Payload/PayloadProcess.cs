...
/// <see cref="PayloadProcess.sceKernelGetAppInfo"/>. The structure layout matches the SDK
/// samples' <c>app_info_t</c> typedef (ps and test_privileges samples).
/// </summary>
...
/// context. Wraps <c>sceKernelGetAppInfo</c> from <c>libkernel</c>, which is the mechanism the
/// SDK <c>ps</c> and <c>test_privileges</c> samples use to read the app id and title id of
